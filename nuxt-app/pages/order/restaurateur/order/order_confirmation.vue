...
export default {
  ...
  computed: {
    filteredOrders() {
      ...
      return filtered.filter((order) => order.order_state === 'CREATED');
    },
  },
  methods: {
    logout() {
    ...
    async deleteOrder(order) {
      try {
        await axios.delete(`http://localhost:3000/deleteOrder/${order._id}`);
    ...
    acceptOrder(order) {
      ...
      // Mettre à jour l'état de la commande en "PREPARED"
      order.order_state = 'PREPARED';
      // Envoyer la mise à jour de la commande
    ...
    async updateOrder(order) {
      try {
        ...
        const restorantId = resto.data.result._id;

        const body = {
          restorant: restorantId,
          order_state: order.order_state,
          paid: order.paid,
          products: order.products.map((product) => ({ id_product: product })),
        ...
        await axios.put(`http://localhost:3000/updateOrder/${orderId}`, body);
        // Mettre à jour la liste des commandes
        this.orders = this.orders.map((o) => (o._id === orderId ? order : o));
      } catch (error) {
        console.error(error);
      } finally {
        // Supprimer l'élément "orderToModify" du localStorage
        localStorage.removeItem('orderToModify');
      }
    },
  },
  async created() {
    if (process.client) {
      ...
      if (token) {
        ...
        try {
          ...
          const response = await axios.get(`http://localhost:3000/getOrdersByRestorantId/${restoId}`);
